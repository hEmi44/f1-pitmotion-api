...
public class TrackerService {
    ...
    public List<TrackerResource> listFuture(Long userId) {
        ...
        return trackerRepo.findByUserId(userId).stream()
                .filter(t -> t.getGrandPrix().getEndingDate() == null
                          || !t.getGrandPrix().getEndingDate().isBefore(today))
                .sorted(Comparator.comparing(t -> t.getGrandPrix().getStartingDate(), Comparator.nullsLast(Comparator.naturalOrder())))
    ...
    public void update(UpdateTrackerRequest req) {
        ...
        UserGpTrackerId id = new UserGpTrackerId(req.userId(), req.trackerId()); // trackerId = gpId
    ...
    public void delete(DeleteTrackerRequest req) {
        User user = userRepo.findById(req.userId()).orElseThrow(() -> new EntityNotFoundException("Utilisateur introuvable"));
